...
3.工行系统收到成功ACK后从用户A中扣款1万元(如果扣款失败就会出现问题)
...
> 这其中是有问题的:
>
> 若第3步中的扣款操作失败，但消息已经成功发送到了Broker。对MQ来说只要消息写入成功，那么这个消息就可以被消费。此建行系统中用户B增加了1万元。出现了数据不一致问题。
...
![image-20220529210614795](./img/50.jpeg)
...
* 返回**ConsumeConcurrentStatus.RECONSUME_LATER**
* 返回Null
* 抛出异常
...
#### 消费不重试配置方式
 ...
 集群消费方式下，消息消费失败后若不希望消费重试，则在捕获异常后同样也返回与消费成功后相同的结果，即**ConsumeConcurrentStatus.CONSUMER_SUCCESS** ,则不进行消费重试。
