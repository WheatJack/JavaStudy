...
* FileChannel
* DatagramChannel
* SocketChannel（TCP传输通道 客户端、服务器都能用）
* ServerSocketChannel（TCP传输通道 多用于服务器）
...
* 通过 FileInputStream 获取的 channel 只能**读**
* 通过 FileOutputStream 获取的 channel 只能**写**
...
* 多路复用
  ...
  > 只需要selector卡住，其他的事件不会卡住，一次性处理完
  >
  > **一个select 可以监听所有的channel的操作事件**
  >
  > 比如selector的时候 监听不同的 channel有各自的read、accept事件等等 那么在处理的接收的是 selectKey会把多个事件返回给select
  >
  > 然后在一个循环里面都处理掉多个事件
